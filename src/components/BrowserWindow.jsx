  ...
  return (
    <Draggable handle=".browser-header" nodeRef={nodeRef}>
      <div 
        ref={nodeRef}
        onMouseDown={onFocus}
        className="absolute top-16 left-28 w-[860px] h-[580px] bg-white border border-gray-300 rounded-t-lg shadow-2xl flex flex-col z-30 overflow-hidden font-mono text-xs select-none"
      >
        {/* Browser Window Title Bar */}
        <div className="browser-header cursor-move bg-gray-100 px-4 py-2.5 border-b border-gray-300 flex justify-between items-center">
        ...
        <div className="bg-gray-50 px-3 py-2 border-b border-gray-200 flex items-center space-x-2 text-gray-700">
          <div className="flex items-center space-x-1">
            <button 
              onClick={handleBack} 
              disabled={historyIndex === 0} 
              className={`p-1.5 rounded bg-white border border-gray-200 hover:bg-gray-100 text-gray-700 ${historyIndex === 0 ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
            ...
            <button 
              ...
              className={`p-1.5 rounded bg-white border border-gray-200 hover:bg-gray-100 text-gray-700 ${historyIndex === history.length - 1 ? 'opacity-40 cursor-not-allowed' : ''}`}
            >
              <ChevronRight className="w-4 h-4" />
            </button>
            <button 
              onClick={() => handleNavigate(currentUrl)} 
              className="p-1.5 rounded bg-white border border-gray-200 hover:bg-gray-100 text-gray-700"
            >
              <RotateCw className="w-3.5 h-3.5" />
            </button>
          </div>

          <div className="flex-1 flex items-center bg-white border border-gray-300 px-3 py-1.5 rounded text-gray-800 shadow-inner space-x-2">
        ...
        {/* Browser Viewport */}
        <div className="flex-1 bg-gray-50 overflow-y-auto p-6 text-gray-700">
          {currentUrl.includes('security-research') ? (
            <div className="space-y-6 max-w-2xl mx-auto">
              <div className="bg-white border border-gray-200 rounded-lg p-5 space-y-3 shadow-sm">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] bg-emerald-50 text-emerald-700 border border-emerald-200 px-2 py-0.5 rounded font-bold uppercase">Active Portfolio Page</span>
                  <span className="text-gray-400 text-[10px]">Munich, DE</span>
                </div>
                ...
                <div className="pt-2 flex gap-2">
                  <button onClick={() => handleNavigate('https://github.com')} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded border border-gray-300 transition-colors flex items-center space-x-1.5 font-medium">
                    <ExternalLink className="w-3.5 h-3.5 text-[#19B6EE]" />
                    <span>View GitHub Repos</span>
                  </button>
                  <button onClick={() => handleNavigate('https://blog.local/writeups')} className="px-3 py-1.5 bg-gray-100 hover:bg-gray-200 text-gray-800 rounded border border-gray-300 transition-colors flex items-center space-x-1.5 font-medium">
            ...
            <div className="space-y-4 max-w-2xl mx-auto text-center py-10">
              <h2 className="text-xl font-bold text-gray-900">GitHub External Redirect Simulated</h2>
              <p className="text-gray-500 text-xs">You are previewing a mock external navigation frame inside the local development environment.</p>
              <button onClick={() => handleNavigate('https://portfolio.local/security-research')} className="px-4 py-2 text-white rounded font-bold" style={{ backgroundColor: currentAccent }}>
                Return to Portfolio
              </button>
            </div>
          ) : (
            <div className="space-y-4 max-w-xl mx-auto text-center py-12">
              ...
              <p className="text-gray-500 text-xs">Firefox can’t establish a connection to the server at <span className="text-amber-600">{currentUrl}</span>.</p>
              <button onClick={() => handleNavigate('https://portfolio.local/security-research')} className="px-4 py-2 bg-gray-200 hover:bg-gray-300 text-gray-800 rounded border border-gray-300 font-bold">
                Go Back Home
              </button>
            </div>
          )}
