...
export default function DolphinWindow({ isOpen, isMinimized, onClose, onMinimize, activeTab, setActiveTab, onFileOpen, onTextEditorOpen }) {
  ...
  // Linux standard directory list for sidebar (Removed dedicated CV tab)
  const places = [
  ...
  const directoryContents = {
    ...
    documents: [
      { name: 'cv.pdf', type: 'file', ext: 'PDF Document', icon: FileText, color: 'text-red-400' },
        ...
        <div className="bg-[#181818] px-3 py-2 border-b border-[#2b2b2b] flex items-center justify-between text-gray-300 text-[11px]">
          <div className="flex items-center space-x-3 flex-1 mr-3">
            <div className="flex items-center space-x-1 bg-[#222222] border border-[#333333] rounded overflow-hidden shrink-0">
              <button onClick={handleBack} disabled={historyIndex === 0} className={`p-1.5 hover:bg-[#333333] transition-colors ${historyIndex === 0 ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}>
                <ChevronLeft className="w-3.5 h-3.5" />
              </button>
              <div className="w-[1px] h-4 bg-[#333333]" />
              <button onClick={handleForward} disabled={historyIndex === history.length - 1} className={`p-1.5 hover:bg-[#333333] transition-colors ${historyIndex === history.length - 1 ? 'opacity-30 cursor-not-allowed' : 'cursor-pointer'}`}>
            ...
            <div className="flex-1 flex items-center space-x-1 bg-[#141414] border border-[#333333] px-3 py-1.5 rounded text-gray-300 shadow-inner overflow-x-auto">
              <span onClick={() => handleSidebarClick('about')} className="hover:text-white cursor-pointer transition-colors">home</span>
              <ChevronRight className="w-3 h-3 text-gray-600 shrink-0" />
              <span onClick={() => handleSidebarClick('about')} className="hover:text-white cursor-pointer transition-colors">guest</span>
              <ChevronRight className="w-3 h-3 text-gray-600 shrink-0" />
          ...
          <div className="flex items-center space-x-1 bg-[#222222] border border-[#333333] rounded p-0.5 shrink-0">
            <button onClick={() => setViewMode('grid')} title="Grid View" className={`p-1.5 rounded transition-colors ${viewMode === 'grid' ? 'bg-[#E95420] text-white' : 'text-gray-400 hover:text-white'}`}>
              <LayoutGrid className="w-3.5 h-3.5" />
            </button>
            <button onClick={() => setViewMode('list')} title="List View" className={`p-1.5 rounded transition-colors ${viewMode === 'list' ? 'bg-[#E95420] text-white' : 'text-gray-400 hover:text-white'}`}>
